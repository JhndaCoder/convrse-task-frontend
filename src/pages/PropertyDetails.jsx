...
const PropertyDetails = () => {
  ...
  useEffect(() => {
    const fetchPropertyAndBooking = async () => {
      try {
        const { data: propertyData } = await customFetch.get(
          `/api/properties/${id}`,
        );
        ...
        if (existingBooking) {
          if (existingBooking.status === "pending") {
            ...
            const paymentIntentResponse = await customFetch.post(
              "/api/bookings/pay",
              {
  ...
  return (
    <div className="property-details">
      ...
      <p className="property-amenities">
        Amenities:{" "}
        {property.amenities.map((amenity) => amenity.toUpperCase()).join(", ")}
      </p>
